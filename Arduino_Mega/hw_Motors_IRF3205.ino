...
class Motor {
  public:
    ...
    void attach(int dir, int pwm, int forward_deadzone_low, int forward_deadzone_high, int backward_deadzone_low, int backward_deadzone_high) {
      InitTimersSafe();
      pin_dir = dir;
      pin_pwm = pwm;
    ...
    void setPinPWM(int pin, int value) {
      ...
      SetPinFrequency(pwm_pseudo, 10000);
...
void attach_motors() {
  ...
  MotorLeft.attach(M_LEFT_DIR, M_LEFT_PWM, 55, 1000, 55, 1000);
  MotorRight.attach(M_RIGHT_DIR, M_RIGHT_PWM, 55, 1000, 55, 1000);
